...
public class AuthorizationServerConfig {
    ...
    public RegisteredClientRepository registeredClientRepository(PasswordEncoder passwordEncoder) {
        RegisteredClient oidcClient = RegisteredClient.withId(UUID.randomUUID().toString())
                ...
                .clientSecret(passwordEncoder.encode("secret")) // En producción esto no iría en código
                .clientAuthenticationMethod(ClientAuthenticationMethod.CLIENT_SECRET_BASIC)
                .authorizationGrantType(AuthorizationGrantType.AUTHORIZATION_CODE)
                .authorizationGrantType(AuthorizationGrantType.REFRESH_TOKEN)
                .redirectUri("http://127.0.0.1:8080/login/oauth2/code/booking-client") // De vuelta al frontend o BFF
    ...
    public AuthorizationServerSettings authorizationServerSettings() {
        return AuthorizationServerSettings.builder()
                .issuer("http://localhost:9000") // El issuer de nuestro Token
                .build();
    }

    /**
     * Inyectamos información extra en el token JWT para el Resource Server
     */
    @Bean
    public OAuth2TokenCustomizer<JwtEncodingContext> jwtTokenCustomizer() {
        return context -> {
            if (context.getPrincipal().getPrincipal() instanceof User userDetails) {
                
                // Extraer roles como una lista de strings
                ...
                if (userDetails.getBusinessId() != null) {
                    context.getClaims().claim("business_id", userDetails.getBusinessId().toString());
                }
